...
//single sign on
function wpe_common_handle_sso()  {

	if(isset($_REQUEST['wpe_token'])) {

		$nonce = sanitize_text_field(  wp_unslash( $_GET['wpe_token'] ) );
		if ( isset( $nonce ) && wp_verify_nonce( $nonce , 'request_nonce') ) {
			setcookie('wpengine_no_cache', $nonce, 60, '', true, true);
			WpeCommon::instance() ->wpe_sso();
		}
	}
}
add_action('admin_init', 'wpe_common_handle_sso');
