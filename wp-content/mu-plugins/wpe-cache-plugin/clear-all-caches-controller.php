...
class ClearAllCachesController {
	...
	public function clear_all_caches() {
		try {
			$this->check_cache_clear_statuses(
				array(
					'memcached' => is_null( $this->wpe_common_adapter->purge_memcached() ),
					'cdn'       => $this->wpe_common_adapter->clear_maxcdn_cache(),
					'varnish'   => $this->wpe_common_adapter->purge_varnish_cache(),
				)
			);
	...
	private function check_cache_clear_statuses( $cache_statuses ) {
		foreach ( $cache_statuses as $cache_type => $cache_status ) {
			if ( false === $cache_status ) {
				throw new \Exception( "Clear cache failure for $cache_type." );
			}
		}
	}
